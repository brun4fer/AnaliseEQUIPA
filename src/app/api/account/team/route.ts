...
export async function POST(request: Request) {
  try {
    ...
    (await cookies()).set(SESSION_COOKIE, createSessionToken({ userId: user.id, username: user.username, role: user.role, mustChangePassword: false, needsOnboarding: false }), sessionCookieOptions);
