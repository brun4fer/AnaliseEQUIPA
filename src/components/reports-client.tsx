...
export function ReportsClient() {
  ...
  async function playClip(clip: Clip) {
    ...
    if (!file) return setNotice(`Load the local video for “${clip.match.title}” first.`);
  ...
  return <div className="space-y-5"><header><p className="text-xs font-bold uppercase tracking-[.22em] text-leaf-400">Aggregated analysis</p><h1 className="mt-2 text-3xl font-bold text-white">Reports and clip export</h1><p className="mt-2 text-sm text-slate-400">Select matches, filter moments, play clips and export them into moment/submoment folders.</p></header>
    ...
    <div className="grid gap-5 xl:grid-cols-[22rem_minmax(0,1fr)]"><Panel className="overflow-hidden"><div className="flex gap-2 border-b border-white/10 p-3"><Button size="sm" onClick={() => setSelectedIds(matches.map((item) => item.id))}><CheckSquare size={14} />Select all</Button><Button size="sm" onClick={() => setSelectedIds([])}>Clear</Button></div><div className="max-h-[42rem] overflow-y-auto">{matches.map((match) => <button key={match.id} onClick={() => toggleMatch(match.id)} className={`flex w-full items-start gap-3 border-b border-white/[.06] p-3 text-left hover:bg-white/[.06] ${selectedIds.includes(match.id) ? "bg-leaf-400/10" : ""}`}>{selectedIds.includes(match.id) ? <CheckSquare size={17} className="text-leaf-400" /> : <Square size={17} className="text-slate-600" />}<span className="min-w-0"><span className="block truncate text-sm font-semibold text-white">{match.title}</span><span className="text-xs text-slate-500">{match.momentCount} moments · {match.video?.fileName || "No video"}</span></span></button>)}</div></Panel>
      <div className="space-y-4"><Panel className="flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-semibold text-white">{loadingDetails ? "Loading clips…" : `${clips.length} clips found`}</p><p className="text-xs text-slate-500">Source videos remain on the computer.</p></div><div className="flex flex-wrap gap-2"><label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg border border-white/10 bg-white/[.06] px-3 text-sm font-semibold text-slate-100 hover:bg-white/[.11]"><Upload size={15} />Load videos<input type="file" accept="video/*" multiple className="hidden" onChange={(event) => void addVideos(event.target.files)} /></label><Button disabled={!clips.length || exporting} onClick={() => void exportClips()}><Archive size={16} />Export clips</Button></div></Panel>
      {playing ? <Panel className="overflow-hidden"><div className="aspect-video bg-black"><video key={playing.url} ref={videoRef} src={playing.url} className="h-full w-full" playsInline onLoadedMetadata={(event) => { event.currentTarget.currentTime = playing.clip.moment.startTimeSeconds; void event.currentTarget.play(); }} onTimeUpdate={(event) => { if (event.currentTarget.currentTime >= playing.clip.moment.endTimeSeconds) event.currentTarget.pause(); }} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} /></div><div className="flex items-center justify-between p-3"><div><p className="text-sm font-semibold text-white">{playing.clip.match.title}</p><p className="text-xs text-slate-500">{playing.clip.moment.momentType.name} · {formatPreciseTime(playing.clip.moment.startTimeSeconds)} – {formatPreciseTime(playing.clip.moment.endTimeSeconds)}</p></div><Button size="icon" variant="primary" onClick={() => isPlaying ? videoRef.current?.pause() : void videoRef.current?.play()}>{isPlaying ? <Pause /> : <Play />}</Button></div></Panel> : null}
      <Panel className="divide-y divide-white/[.06] overflow-hidden">{clips.length ? clips.map((clip) => <button key={clip.moment.id} onClick={() => void playClip(clip)} className="flex w-full items-center gap-3 p-3 text-left hover:bg-white/[.05]"><Play size={15} className="text-leaf-400" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-white">{clip.match.title}</span><span className="text-xs text-slate-500">{clip.moment.momentType.name} · {formatPreciseTime(clip.moment.startTimeSeconds)} – {formatPreciseTime(clip.moment.endTimeSeconds)}</span></span><Badge>{clip.moment.subMoments.length} sub.</Badge></button>) : <div className="p-10 text-center text-sm text-slate-500"><FileVideo className="mx-auto mb-3" />Select at least one match.</div>}</Panel></div></div>
  </div>;
}
