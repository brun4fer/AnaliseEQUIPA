  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4" role="dialog" aria-modal="true" aria-label="Edit moment">
    <Panel className="w-full max-w-lg p-5"><div className="flex items-center justify-between"><div><Label>Edit moment</Label><h2 className="mt-1 text-lg font-bold text-white">{moment.momentType.name}</h2></div><Button size="icon" variant="ghost" aria-label="Close" onClick={onClose}><X size={17} /></Button></div>
      <form className="mt-5 space-y-4" onSubmit={submit}>
        ...
        <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-2"><Label>Start time in seconds</Label><Input type="number" min={0} max={duration || undefined} step={.1} value={start} onChange={(event) => setStart(event.target.value)} /><Button type="button" size="sm" onClick={() => setStart(String(roundTime(currentTime)))}><Clock size={13} />Use {formatTime(currentTime)}</Button></label><label className="grid gap-2"><Label>End time in seconds</Label><Input type="number" min={0} max={duration || undefined} step={.1} value={end} onChange={(event) => setEnd(event.target.value)} /><Button type="button" size="sm" onClick={() => setEnd(String(roundTime(currentTime)))}><Clock size={13} />Use {formatTime(currentTime)}</Button></label></div>
