...
export function SubmomentWorkspace({ matchId }: { matchId: string }) {
  ...
  useEffect(() => {
    if (loading) return;
    const node = playerColumnRef.current;
    if (!node) return;
    const measure = () => setPlayerColumnHeight(Math.ceil(node.getBoundingClientRect().height));
