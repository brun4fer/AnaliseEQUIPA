...
export function SubmomentWorkspace({ matchId }: { matchId: string }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid min-w-[1320px] grid-cols-[19rem_minmax(34rem,1fr)_27rem] items-start gap-4">
      ...
      <div className="space-y-4">
        <Panel className="overflow-hidden"><div className="relative aspect-video bg-black">{sourceUrl ? <video ref={videoRef} src={sourceUrl} className="h-full w-full" playsInline onTimeUpdate={handleTimeUpdate} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} /> : <div className="flex h-full flex-col items-center justify-center p-8 text-center"><FileVideo size={50} className="text-leaf-400" /><h2 className="mt-4 font-bold text-white">{restoringVideo ? "Restoring the local video…" : "Select the video again"}</h2><p className="mt-2 max-w-md text-sm text-slate-500">Large videos remain available between pages while this browser tab stays open.</p>{match.video ? <div className="mt-4 rounded-lg border border-white/10 bg-white/[.05] px-3 py-2 text-xs text-slate-400">Expected: {match.video.fileName} · {formatBytes(match.video.fileSize)}</div> : null}{!restoringVideo ? <Button className="mt-5" variant="primary" onClick={() => fileInputRef.current?.click()}><Upload size={16} />Choose video</Button> : null}</div>}</div><div className="flex items-center justify-between border-t border-white/10 p-3"><div className="flex gap-2"><Button size="icon" disabled={selectedIndex <= 0} onClick={() => selectMoment(moments[selectedIndex - 1])}><ChevronLeft /></Button><Button size="icon" variant="primary" disabled={!sourceUrl || !selectedMoment} onClick={togglePlayback}>{playing ? <Pause /> : <Play />}</Button><Button size="icon" disabled={selectedIndex < 0 || selectedIndex >= moments.length - 1} onClick={() => selectMoment(moments[selectedIndex + 1])}><ChevronRight /></Button></div><span className="font-mono text-sm text-white">{formatTime(currentTime)} {selectedMoment ? <span className="text-slate-600">/ {formatTime(selectedMoment.endTimeSeconds)}</span> : null}</span></div></Panel>
