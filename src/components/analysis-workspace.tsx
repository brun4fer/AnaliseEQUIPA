...
export function AnalysisWorkspace({ matchId }: { matchId: string }) {
  ...
  return <div className="flex min-h-0 flex-col gap-2 xl:h-[calc(100dvh-6.5rem)] xl:overflow-hidden">
    ...
    <Panel className="flex shrink-0 items-stretch overflow-hidden">
      ...
      <div className="flex shrink-0 items-center border-l border-white/10 px-2"><Button size="sm" className="mr-1 h-8 whitespace-nowrap" variant="secondary" disabled={uploading} onClick={() => void openCloudLibrary()}><Cloud size={13} />Cloud library</Button><Button size="sm" className="h-8 whitespace-nowrap" variant={uploading ? "danger" : "secondary"} onClick={() => uploading ? cancelUpload() : fileInputRef.current?.click()}>{uploading ? <Loader2 size={13} className="animate-spin" /> : <Upload size={13} />}{uploading ? `Cancel ${Math.round(uploadProgress * 100)}%` : match.video?.storageStatus === "READY" ? "Replace video" : "Upload video"}</Button></div>
    </Panel>
      ...
      <Panel className="order-2 flex min-h-0 min-w-0 flex-col overflow-hidden">
        ...
        <div className="shrink-0 border-t border-white/10 bg-pitch-950/90 p-2">
          ...
          <div className="mt-1.5 flex items-center justify-between gap-2 overflow-x-auto">
            ...
            <span className="inline-flex shrink-0 items-center gap-1 font-mono text-xs text-white"><Clock3 size={13} className="text-cyan-200" />{formatTime(currentTime)} / {formatTime(duration)}</span>
          </div>
        </div>
      </Panel>
